...
def create_question(question_text, days):
    ...
    given number of `days` offset to now (negative for questions published in the 
...
class QuestionIndexViewTests(TestCase):
    def test_no_question(self):
        """
        If no question exist, an appropiated messaged is displayed
        """
    ...
    def test_future_question(self):
        """
        Questions with a pub_date set in the future aren't displayed on the index page 
        """
    ...
    def test_future_question_and_past_question(self):
        """
        Even if both past and future question exist, only past questions are display
        """
        question = create_question("Past question.", -30)
        create_question("Future question", 30)
        response = self.client.get(reverse("polls:index"))
...
class QuestionModelTests(TestCase):
    def test_was_published_recently_with_future_question(self):
        ...
        time = timezone.now() + datetime.timedelta(days=38)
